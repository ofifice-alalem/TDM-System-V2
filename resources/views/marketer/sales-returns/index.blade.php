                ...
                <details class="bg-white dark:bg-dark-card rounded-2xl shadow-lg shadow-gray-200/50 dark:shadow-none border border-gray-200 dark:border-dark-border mb-6" style="z-index: 1000; position: relative;">
                    ...
                    <form method="GET" action="{{ route('marketer.sales-returns.index') }}" class="p-4 pt-2">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                            ...
                            <div style="z-index: 1000; position: relative;" id="return-store-wrapper">
                                <label class="block text-xs font-bold text-gray-600 dark:text-gray-400 mb-1.5">المتجر</label>
                                <input type="text" id="return-store-search" autocomplete="off" placeholder="ابحث عن متجر..." value="{{ request('store') }}"
                                ...
                                <input type="hidden" name="store" id="return-store-value" value="{{ request('store') }}">
                                <div id="return-store-dropdown" class="hidden absolute z-[9999] w-full mt-1 bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-xl shadow-xl max-h-52 overflow-y-auto"></div>
                            </div>
                        </div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const storeSearch   = document.getElementById('return-store-search');
    const storeValue    = document.getElementById('return-store-value');
    const storeDropdown = document.getElementById('return-store-dropdown');
    const storeWrapper  = document.getElementById('return-store-wrapper');
    ...
    storeSearch.addEventListener('input', function () {
        ...
        const results = returnStoresData.filter(s =>
        ...
        storeDropdown.innerHTML = results.map(s => `
            <div class="return-store-option px-4 py-2.5 hover:bg-gray-100 dark:hover:bg-dark-bg cursor-pointer border-b border-gray-100 dark:border-dark-border last:border-0" data-name="${s.name}">
        ...
        storeDropdown.querySelectorAll('.return-store-option').forEach(opt => {
