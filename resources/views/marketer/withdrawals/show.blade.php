            ...
            <div class="lg:col-span-4 space-y-6 animate-slide-up" style="animation-delay: 0.1s">
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-dark-border z-10 relative space-y-3">
                        ...
                        @if($withdrawal->status === 'pending')
                            ...
                            <div 
                                ...
                                <form action="{{ route('marketer.withdrawals.cancel', $withdrawal) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                ...
                <div class="bg-white dark:bg-dark-card rounded-[1.5rem] border border-gray-200 dark:border-dark-border p-6 shadow-lg shadow-gray-200/50 dark:shadow-sm">
                    <h3 class="font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
                        <i data-lucide="list" class="w-5 h-5 text-amber-500"></i>
                        سجل العمليات
                    </h3>
                    ...
                    <div class="relative space-y-6 before:absolute before:inset-0 before:mr-[19px] before:h-full before:w-0.5 before:bg-gradient-to-b before:from-gray-200 dark:before:from-dark-border before:via-gray-100 dark:before:via-dark-bg before:to-transparent">
                        ...
                        <div class="relative flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-dark-muted mt-1">بواسطة: {{ $withdrawal->marketer->full_name }}</p>
                                <span class="text-[10px] bg-gray-100 dark:bg-dark-bg px-2 py-0.5 rounded text-gray-500 dark:text-dark-muted mt-2 inline-block font-mono">{{ $withdrawal->created_at->format('Y-m-d h:i A') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="relative flex items-start gap-4 animate-slide-up">
                            ...
                            <div>
                                <h4 class="font-bold text-gray-900 dark:text-white text-sm">تمت الموافقة</h4>
                                <p class="text-xs text-gray-500 dark:text-dark-muted mt-1">بواسطة: {{ $withdrawal->approver->full_name }}</p>
                                <span class="text-[10px] bg-gray-100 dark:bg-dark-bg px-2 py-0.5 rounded text-gray-500 dark:text-dark-muted mt-2 inline-block font-mono">{{ $withdrawal->approved_at->format('Y-m-d h:i A') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="relative flex items-start gap-4 animate-slide-up">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 dark:text-dark-muted mt-1">بواسطة: {{ $withdrawal->rejecter->full_name ?? 'الإدارة' }}</p>
                                <span class="text-[10px] bg-gray-100 dark:bg-dark-bg px-2 py-0.5 rounded text-gray-500 dark:text-dark-muted mt-2 inline-block font-mono">{{ $withdrawal->rejected_at->format('Y-m-d h:i A') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="relative flex items-start gap-4 animate-slide-up">
                            ...
                            <div>
                                ...
                                <span class="text-[10px] bg-gray-100 dark:bg-dark-bg px-2 py-0.5 rounded text-gray-500 dark:text-dark-muted mt-2 inline-block font-mono">{{ $withdrawal->updated_at->format('Y-m-d h:i A') }}</span>
                            </div>
                        </div>
                        @else
                        <div class="relative flex items-start gap-4 opacity-50 grayscale">
                            ...
                            <div>
                                <h4 class="font-bold text-gray-400 dark:text-gray-600 text-sm">الموافقة</h4>
