...
<div class="min-h-screen py-8">
    <div class="max-w-7xl mx-auto space-y-8 px-3 lg:px-8">
        ...
        <form action="{{ route('marketer.returns.store') }}" method="POST" class="space-y-4 md:space-y-6">
            @csrf
            ...
            <div class="bg-white dark:bg-dark-card rounded-[2rem] p-3 md:p-8 shadow-xl shadow-gray-200/60 dark:shadow-none border border-gray-200 dark:border-dark-border relative overflow-hidden animate-slide-up">
                <div class="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 dark:text-dark-muted mt-2 mr-14 font-medium">اختر المنتجات من مخزونك الفعلي</p>
                    </div>
                </div>
                            ...
                            <div class="md:col-span-6">
                                ...
                                <select name="items[0][product_id]" class="product-select w-full bg-white dark:bg-dark-card border border-gray-200 dark:border-dark-border rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all" required>
                                    ...
                                    @foreach($stock as $item)
                                        <option value="{{ $item->product_id }}" data-stock="{{ $item->quantity }}">
                                            {{ $item->product->name }} (متوفر: {{ $item->quantity }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
            ...
            <div class="bg-white dark:bg-dark-card rounded-[2rem] p-3 md:p-8 shadow-xl shadow-gray-200/60 dark:shadow-none border border-gray-200 dark:border-dark-border animate-slide-up" style="animation-delay: 0.1s">
                ...
                <textarea name="notes" rows="4" class="w-full bg-gray-50 dark:bg-dark-bg border border-gray-200 dark:border-dark-border rounded-xl px-4 py-3 text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-primary-500 transition-all" placeholder="سبب الإرجاع أو أي ملاحظات إضافية..."></textarea>
            </div>
            ...
            <div class="flex gap-4 animate-slide-up mt-6" style="animation-delay: 0.2s">
                ...
                <a href="{{ route('marketer.returns.index') }}" class="flex-1 px-8 py-4 bg-gray-500 hover:bg-gray-600 text-white rounded-xl text-sm md:text-base font-bold transition-all shadow-lg shadow-gray-200/50 dark:shadow-none flex items-center justify-center gap-2">
