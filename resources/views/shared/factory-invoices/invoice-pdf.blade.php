...
<head>
    ...
    <title>فاتورة مصنع {{ $invoiceNumber }}</title>
    <style>
        @font-face {
            font-family: 'Cairo';
...
<body>
    ...
    <div class="container">
        ...
        <div class="info-section">
            <div class="info-row">
                <div>{{ $keeperName }} :<span class="info-label">{!! $labels['keeper'] !!}</span></div>
                <div>{{ $date }} :<span class="info-label">{!! $labels['date'] !!}</span></div>
            </div>
            <div class="info-row">
                <div>{{ $status }} :<span class="info-label">{!! $labels['status'] !!}</span></div>
                @if(isset($documentedBy))
                <div>{{ $documentedBy }} :<span class="info-label">{!! $labels['documentedBy'] !!}</span></div>
                @elseif(isset($cancelledBy))
                <div>{{ $cancelledBy }} :<span class="info-label">{!! $labels['cancelledBy'] !!}</span></div>
                @else
                <div>{{ $itemsCount }} :<span class="info-label">{!! $labels['itemsCount'] !!}</span></div>
                @endif
            </div>
        </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr style="background: #f5f5f5; font-weight: bold;">
                    <td style="background: #eee; font-size: 14px;">{{ $totalQuantity }}</td>
                    <td colspan="2" style="background: #eee; text-align: right; font-size: 14px;">{!! $labels['totalQuantity'] !!}</td>
                </tr>
            </tbody>
        </table>

        <div class="signatures">
            <div class="signature"><span>{!! $labels['management'] !!}</span></div>
            <div class="signature"><span>{!! $labels['keeperSignature'] !!}</span></div>
        </div>
    </div>
</body>
</html>
