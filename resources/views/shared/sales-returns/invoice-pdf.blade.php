...
<head>
    ...
    <title>إرجاع {{ $returnNumber }}</title>
    <style>
        @font-face {
            font-family: 'Cairo';
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Cairo', 'DejaVu Sans', sans-serif; }
        ...
        .signature { display: table-cell; width: 33.33%; text-align: center; padding: 0 10px; font-weight: bold; }
        .signature span { display: inline-block; padding-top: 10px; border-top: 2px dotted #000; min-width: 120px; }
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="header-right">
                ...
                <div class="invoice-number">#{{ $returnNumber }}</div>
            </div>
        </div>
        ...
        <div class="info-section">
            ...
            <div class="info-row">
                ...
                <div>#{{ $invoiceNumber }} :<span class="info-label">{!! $labels['originalInvoice'] !!}</span></div>
            </div>
            <div class="info-row">
                <div>{{ $status }} :<span class="info-label">{!! $labels['status'] !!}</span></div>
                @if($keeperName && $confirmedDate)
                <div>{{ $keeperName }} ({{ $confirmedDate }}) :<span class="info-label">{!! $statusValue === 'approved' ? $labels['approvedBy'] : $labels['rejectedBy'] !!}</span></div>
                @endif
            </div>
        </div>
        ...
        <table>
            ...
            <tbody>
                ...
                <tr>
                    <td>{!! $labels['currency'] !!} {{ $item->total_price }}</td>
                    <td>{!! $labels['currency'] !!} {{ $item->unit_price }}</td>
                    <td>{{ $item->quantity }}</td>
                    <td>{{ $item->name }}</td>
                    <td>{{ $index + 1 }}</td>
                </tr>
        ...
        <div class="totals">
            <div class="totals-row final">
                {!! $labels['currency'] !!} {{ $totalAmount }} : <strong>{!! $labels['finalTotal'] !!}</strong>
            </div>
        </div>
