...
<div class="bg-white dark:bg-dark-card rounded-2xl p-2 shadow-lg shadow-gray-200/50 dark:shadow-none border border-gray-200 dark:border-dark-border mb-6">
    <div class="flex flex-wrap gap-2">
        <a href="{{ $route(['status' => 'pending']) }}" class="{{ !request('all') && (!request('status') || request('status') === 'pending') ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-2 border-amber-200 dark:border-amber-800' : 'bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-dark-border' }} px-3 py-2.5 rounded-xl font-bold transition-all text-sm flex items-center gap-1.5 flex-1 basis-[calc(33.333%-0.5rem)] md:basis-auto justify-center">
            <i data-lucide="clock" class="w-4 h-4"></i>
            قيد الانتظار
        </a>
        <a href="{{ $route(['status' => 'approved']) }}" class="{{ request('status') === 'approved' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border-2 border-blue-200 dark:border-blue-800' : 'bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-dark-border' }} px-3 py-2.5 rounded-xl font-bold transition-all text-sm flex items-center gap-1.5 flex-1 basis-[calc(33.333%-0.5rem)] md:basis-auto md:flex-[1.2] justify-center">
            <i data-lucide="check-circle" class="w-4 h-4"></i>
            تمت الموافقة
        </a>
        <a href="{{ $route(['status' => 'documented']) }}" class="{{ request('status') === 'documented' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-2 border-emerald-200 dark:border-emerald-800' : 'bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-dark-border' }} px-3 py-2.5 rounded-xl font-bold transition-all text-sm flex items-center gap-1.5 flex-1 basis-[calc(33.333%-0.5rem)] md:basis-auto justify-center">
