...
@section('title', 'إدارة طلب الإرجاع #' . $request->invoice_number)
                ...
                <div class="bg-white dark:bg-dark-card rounded-[2rem] p-4 md:p-8 shadow-xl shadow-gray-200/60 dark:shadow-none border border-gray-200 dark:border-dark-border relative overflow-hidden">
                    <div class="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-dark-muted mt-2 mr-14 font-medium">قائمة الأصناف المراد استلامها</p>
                        </div>
                        ...
                        <table class="w-full border-separate border-spacing-y-3">
                            <thead>
                                <tr class="text-xs text-gray-400 dark:text-dark-muted font-bold uppercase tracking-wider">
                                    ...
                                    <th class="px-4 md:px-6 py-2 text-center w-[35%]">الكمية المرجعة</th>
                                </tr>
                            </thead>
                            <tbody>
                    ...
                    <div class="mt-8 bg-gray-900 dark:bg-black/40 rounded-3xl p-8 shadow-xl shadow-gray-200 dark:shadow-none text-white flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden group">
                        <div class="absolute top-0 left-0 w-64 h-64 bg-red-500 dark:bg-red-500 rounded-full mix-blend-overlay dark:mix-blend-screen filter blur-[60px] opacity-20 dark:opacity-10 -translate-x-1/2 -translate-y-1/2 transition-transform duration-700 group-hover:scale-125"></div>
                        
                        <div class="flex items-center gap-5 relative z-10">
                            <div class="w-16 h-16 bg-white/5 rounded-2xl flex items-center justify-center text-red-400 dark:text-red-400 border border-white/10 shadow-inner">
                                <i data-lucide="layers" class="w-8 h-8"></i>
                            </div>
                            <div>
                                <div class="text-base text-gray-100 dark:text-gray-200 font-bold uppercase tracking-wider mb-1">إجمالي القطع المرجعة</div>
                                <div class="text-xs text-gray-500 font-medium">مجموع الكميات في هذا الإرجاع</div>
                            </div>
                        </div>
            ...
            <div class="lg:col-span-4 space-y-6 animate-slide-up order-1 lg:order-2" style="animation-delay: 0.1s">
                ...
                <div class="bg-gray-50 dark:bg-dark-card/50 rounded-[1.5rem] border-2 border-dashed border-gray-200 dark:border-dark-border p-6 relative overflow-hidden">
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-dark-border z-10 relative">
                        ...
                        @if($request->status === 'pending')
                        <div class="space-y-3">
                         <form action="{{ route('warehouse.returns.approve', $request) }}" method="POST">
                            @csrf
                            @method('PATCH')
                        ...
                        <div x-data="{ showReject: false }" class="mt-3">
                            ...
                            <div x-show="showReject" x-transition class="bg-red-50 dark:bg-red-900/10 rounded-2xl p-4 border border-red-100 dark:border-red-900/30">
                                <form action="{{ route('warehouse.returns.reject', $request) }}" method="POST">
                                    @csrf
                                    @method('PATCH')
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-dark-border z-10 relative">
                        <h4 class="font-bold text-gray-900 dark:text-white text-sm mb-4">توثيق الاستلام</h4>
                         <form action="{{ route('warehouse.returns.document', $request) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                            @csrf
                            <div class="relative group">
                                <label for="file-upload" id="file-label" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-2xl cursor-pointer bg-gray-50 dark:bg-dark-bg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                        ...
                                        <p id="upload-text" class="text-xs text-gray-500 dark:text-gray-400"><span class="font-bold">ارفق صورة</span> الفاتورة المختومة</p>
                                    </div>
