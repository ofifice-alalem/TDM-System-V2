            ...
            <div class="lg:col-span-8">
                ...
                <div class="bg-white dark:bg-dark-card rounded-2xl p-2 shadow-lg shadow-gray-200/50 dark:shadow-none border border-gray-200 dark:border-dark-border mb-6">
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('warehouse.requests.index', ['status' => 'pending']) }}" class="{{ !request('all') && (!request('status') || request('status') === 'pending') ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 border-2 border-amber-200 dark:border-amber-800' : 'bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-dark-border' }} px-3 py-2.5 rounded-xl font-bold transition-all text-sm flex items-center gap-1.5 flex-1 basis-[calc(33.333%-0.5rem)] md:basis-auto justify-center">
                            <i data-lucide="clock" class="w-4 h-4"></i>
                            قيد الانتظار
                        </a>
                        <a href="{{ route('warehouse.requests.index', ['status' => 'approved']) }}" class="{{ request('status') === 'approved' ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-400 border-2 border-blue-200 dark:border-blue-800' : 'bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-dark-border' }} px-3 py-2.5 rounded-xl font-bold transition-all text-sm flex items-center gap-1.5 flex-1 basis-[calc(33.333%-0.5rem)] md:basis-auto md:flex-[1.2] justify-center">
                            <i data-lucide="check-circle" class="w-4 h-4"></i>
                            تمت الموافقة
                        </a>
                        <a href="{{ route('warehouse.requests.index', ['status' => 'documented']) }}" class="{{ request('status') === 'documented' ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 border-2 border-emerald-200 dark:border-emerald-800' : 'bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-dark-border' }} px-3 py-2.5 rounded-xl font-bold transition-all text-sm flex items-center gap-1.5 flex-1 basis-[calc(33.333%-0.5rem)] md:basis-auto justify-center">
                            <i data-lucide="file-check" class="w-4 h-4"></i>
                            موثق
                        </a>
                        <a href="{{ route('warehouse.requests.index', ['status' => 'rejected']) }}" class="{{ request('status') === 'rejected' ? 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 border-2 border-red-200 dark:border-red-800' : 'bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-dark-border' }} px-3 py-2.5 rounded-xl font-bold transition-all text-sm flex items-center gap-1.5 flex-1 basis-[calc(33.333%-0.5rem)] md:basis-auto justify-center">
                            <i data-lucide="x-circle" class="w-4 h-4"></i>
                            مرفوض
                        </a>
                        <a href="{{ route('warehouse.requests.index', ['status' => 'cancelled']) }}" class="{{ request('status') === 'cancelled' ? 'bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 border-2 border-gray-300 dark:border-gray-600' : 'bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-dark-border' }} px-3 py-2.5 rounded-xl font-bold transition-all text-sm flex items-center gap-1.5 flex-1 basis-[calc(33.333%-0.5rem)] md:basis-auto justify-center">
                            <i data-lucide="slash" class="w-4 h-4"></i>
                            ملغي
                        </a>
                        <a href="{{ route('warehouse.requests.index', ['all' => '1']) }}" class="{{ request('all') ? 'bg-primary-600 text-white' : 'bg-gray-100 dark:bg-dark-bg text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-dark-border' }} px-3 py-2.5 rounded-xl font-bold transition-all text-sm flex items-center gap-1.5 flex-1 basis-[calc(33.333%-0.5rem)] md:basis-auto justify-center">
                        ...
                        <div class="flex-1 p-4 md:p-6">
                            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                <div class="flex-1">
                                    ...
                                    <div class="mb-2">
                                        <span class="inline-flex items-center gap-1.5 {{ $statusConfig['bg'] }} {{ $statusConfig['text'] }} px-3 py-1.5 rounded-lg text-sm font-bold border {{ str_replace('bg-', 'border-', $statusConfig['iconBg']) }}">
                                ...
                                <div class="flex gap-2">
                                    <a href="{{ route('warehouse.requests.show', $request) }}" class="px-5 py-2.5 bg-white dark:bg-dark-card border-2 border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 rounded-xl font-bold hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-400 dark:hover:border-gray-500 transition-all text-sm flex items-center gap-2 shadow-sm">
