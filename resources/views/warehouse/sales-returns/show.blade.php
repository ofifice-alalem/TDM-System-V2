                ...
                <div class="bg-white dark:bg-dark-card rounded-[2rem] p-4 md:p-8 shadow-xl shadow-gray-200/60 dark:shadow-none border border-gray-200 dark:border-dark-border relative overflow-hidden">
                    <div class="flex items-center justify-between mb-8">
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-dark-muted mt-2 mr-14 font-medium">بيانات المسوق والمتجر والفاتورة</p>
                        </div>
                    </div>
                    <div class="bg-gray-50/50 dark:bg-dark-bg/60 rounded-2xl p-6 border border-gray-100 dark:border-dark-border">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <p class="font-black text-gray-900 dark:text-white text-lg">{{ $salesReturn->marketer->full_name }}</p>
                            </div>
                            <div>
                                <p class="text-xs text-gray-400 dark:text-dark-muted mb-2 font-bold uppercase tracking-wider">المتجر</p>
            ...
            <div class="lg:col-span-4 space-y-6 animate-slide-up order-1 lg:order-2" style="animation-delay: 0.1s">
                ...
                <div class="bg-gray-50 dark:bg-dark-card/50 rounded-[1.5rem] border-2 border-dashed border-gray-200 dark:border-dark-border p-6 relative overflow-hidden">
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-dark-border z-10 relative">
                        <a href="{{ route('warehouse.sales-returns.pdf', $salesReturn) }}" target="_blank" class="w-full bg-gray-900 dark:bg-dark-bg text-white hover:bg-gray-800 dark:hover:bg-dark-card border border-transparent dark:border-dark-border py-3.5 rounded-xl font-bold transition-all shadow-lg shadow-gray-200 dark:shadow-none flex items-center justify-center gap-2 group mb-4">
                            <i data-lucide="printer" class="w-5 h-5 group-hover:scale-110 transition-transform"></i>
                            طباعة PDF
                        </a>

                        @if($salesReturn->status === 'pending')
                            <form action="{{ route('warehouse.sales-returns.approve', $salesReturn->id) }}" method="POST" enctype="multipart/form-data" class="mb-4">
                                @csrf
                                <div class="mb-4">
                                    <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-3">صورة الفاتورة المختومة</label>
                                    <div class="relative">
                                        <input type="file" name="stamped_image" id="stamped_image" accept="image/*" class="hidden">
                                        <label for="stamped_image" class="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 dark:border-dark-border rounded-xl cursor-pointer bg-gray-50 dark:bg-dark-bg hover:bg-gray-100 dark:hover:bg-dark-card transition-all group">
                                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                                ...
                                                <p class="text-xs text-gray-400 dark:text-gray-500">PNG, JPG أو JPEG</p>
                                            </div>
                                        </label>
                                        <div id="preview" class="hidden mt-3 relative">
                                            ...
                                            <button type="button" onclick="document.getElementById('stamped_image').value=''; document.getElementById('preview').classList.add('hidden')" class="absolute top-2 right-2 bg-red-500 hover:bg-red-600 text-white rounded-full p-1.5 shadow-lg transition-colors">
                            ...
                            <div x-data="{ showReject: false }">
                                ...
                                <div 
                                    ...
                                    <form action="{{ route('warehouse.sales-returns.reject', $salesReturn->id) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                ...
                <div class="bg-white dark:bg-dark-card rounded-[1.5rem] border border-gray-200 dark:border-dark-border p-6 shadow-lg shadow-gray-200/50 dark:shadow-sm">
                    ...
                    <div class="relative space-y-6 before:absolute before:inset-0 before:mr-[19px] before:h-full before:w-0.5 before:bg-gradient-to-b before:from-gray-200 dark:before:from-dark-border before:via-gray-100 dark:before:via-dark-bg before:to-transparent">
                        ...
                        <div class="relative flex items-start gap-4">
                            ...
                            <div>
                                <h4 class="font-bold text-gray-900 dark:text-white text-sm">تم إنشاء طلب الإرجاع</h4>
                                <p class="text-xs text-gray-500 dark:text-dark-muted mt-1">بواسطة: {{ $salesReturn->marketer->full_name }}</p>
                                <span class="text-[10px] bg-gray-100 dark:bg-dark-bg px-2 py-0.5 rounded text-gray-500 dark:text-dark-muted mt-2 inline-block font-mono">{{ $salesReturn->created_at->format('Y-m-d h:i A') }}</span>
                            </div>
                        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const input = document.getElementById('stamped_image');
        if (input) {
