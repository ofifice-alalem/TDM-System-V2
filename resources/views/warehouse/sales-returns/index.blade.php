        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            <div class="lg:col-span-8">
                ...
                <div class="bg-white dark:bg-dark-card rounded-2xl p-4 shadow-lg shadow-gray-200/50 dark:shadow-none border border-gray-200 dark:border-dark-border mb-6">
                    <form method="GET" action="{{ route('warehouse.sales-returns.index') }}" class="flex flex-col md:flex-row gap-3">
            ...
            @forelse($returns as $return)
                @include('shared.sales-returns._return-card', [
                    'return' => $return,
                    'slot' => '<div class="flex items-center gap-2 mb-2"><i data-lucide="store" class="w-4 h-4 text-gray-400"></i><span class="text-sm font-bold text-gray-700 dark:text-gray-300">' . $return->store->name . '</span></div>
                              <div class="flex items-center gap-2 text-sm"><i data-lucide="user" class="w-4 h-4 text-gray-400"></i><span class="text-gray-600 dark:text-gray-400">المسوق: ' . $return->marketer->full_name . '</span></div>',
