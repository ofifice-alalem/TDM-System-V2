...
<div class="py-8">
    <div class="max-w-7xl mx-auto px-4">
        ...
        {{-- قسم: الطلبات العاجلة (عرض كامل في الأسفل) --}}
        @if($oldestRequests->count() > 0)
        <div class="bg-gradient-to-br from-red-50 to-orange-50 dark:from-red-900/20 dark:to-orange-900/20 rounded-2xl p-6 border-2 border-red-200 dark:border-red-700/30 mt-6 shadow-lg">
            <div class="flex items-center gap-3 mb-5">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                @foreach($oldestRequests as $request)
                    @php
                        $diff = now()->diff($request->created_at);
                        $days = $diff->days;
                        $hours = $diff->h;
                        $timeText = $days > 0 ? "$days يوم و $hours ساعة" : "$hours ساعة";
                    @endphp
                    
                    <div class="bg-white dark:bg-dark-card rounded-xl p-4 border border-red-200 dark:border-red-700/50 hover:shadow-xl transition-all hover:-translate-y-1">
                        <div class="flex items-center justify-between mb-3">
                            <span class="font-bold text-gray-900 dark:text-white">{{ $request->invoice_number }}</span>
                            <span class="bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-2 py-1 rounded text-xs font-bold">
                                {{ $timeText }}
                            </span>
                        </div>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mb-3">{{ $request->marketer->full_name }}</p>
                        <a href="{{ route('warehouse.requests.show', $request) }}" class="block w-full text-center px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg font-bold text-sm transition-all">
                            معالجة فوراً
                        </a>
                    </div>
                @endforeach
