                            ...
                            <div class="flex gap-2">
                                ...
                                @if(!$isExpired)
                                    <form action="{{ route('admin.discounts.toggle', $discount) }}" method="POST">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="h-[88px] px-5 {{ $discount->is_active ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-400 hover:bg-amber-200' : 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 hover:bg-green-200' }} rounded-xl font-bold transition-all text-sm flex items-center gap-2 shadow-sm">
                                            <i data-lucide="{{ $discount->is_active ? 'pause' : 'play' }}" class="w-4 h-4"></i>
                                            {{ $discount->is_active ? 'تعطيل' : 'تفعيل' }}
                                        </button>
                                    </form>
                                @else
                                    <span class="h-[88px] px-5 bg-gray-100 dark:bg-gray-800 text-gray-400 dark:text-gray-500 rounded-xl font-bold text-sm flex items-center gap-2">
