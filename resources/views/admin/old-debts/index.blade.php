...
<div class="min-h-screen py-8" x-data="{
    ...
    <div class="max-w-[1600px] mx-auto space-y-8 px-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fade-in-down">
            ...
            <div class="lg:col-span-4 lg:translate-y-[30px]">
                <a href="{{ route('admin.old-debts.create') }}" class="px-8 py-4 bg-primary-600 hover:bg-primary-700 text-white rounded-xl font-bold transition-all shadow-lg shadow-primary-200/50 dark:shadow-none flex items-center justify-center gap-2 w-full">
                ...
                <details class="bg-white dark:bg-dark-card rounded-2xl shadow-lg shadow-gray-200/60 dark:shadow-none border border-gray-200 dark:border-dark-border mb-6 animate-slide-up">
                    <summary class="px-4 py-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-dark-bg/50 transition-colors rounded-2xl flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            ...
                            @if(request('store_id') || request('from_date') || request('to_date'))
                    ...
                    <form method="GET" action="{{ route('admin.old-debts.index') }}" class="p-4 pt-2">
                        ...
                        <div class="flex gap-3">
                            ...
                            @if(request('store_id') || request('from_date') || request('to_date'))
                                <a href="{{ route('admin.old-debts.index') }}" class="px-6 py-2.5 bg-gray-500 hover:bg-gray-600 text-white rounded-xl font-bold transition-all flex items-center gap-2">
                ...
                <div class="bg-white dark:bg-dark-card rounded-[2rem] p-4 shadow-xl shadow-gray-200/60 dark:shadow-none border border-gray-200 dark:border-dark-border animate-slide-up">
                        ...
                        <div class="bg-gradient-to-br from-gray-50 to-white dark:from-dark-bg dark:to-dark-card rounded-2xl border-2 border-orange-200 dark:border-orange-800/40 mb-4 p-5 hover:shadow-lg transition-all">
                            ...
                            <div class="mt-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                <div class="flex items-center gap-2">
                                    ...
                                    <span class="font-bold text-gray-900 dark:text-white">{{ $debt->store->name }}</span>
                                </div>
                    ...
                    @empty
                        <div class="text-center py-12">
                            ...
                            <p class="text-gray-500 dark:text-dark-muted">لم يتم تسجيل أي ديون سابقة بعد</p>
                        </div>
                    @endforelse
                    ...
                    <div class="space-y-6">
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-dark-muted leading-relaxed">تسجيل الأرصدة المتراكمة على المتاجر قبل تشغيل النظام الحالي</p>
                            </div>
                        </div>
                        <div class="flex items-start gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-dark-muted leading-relaxed">تُرقَّم الفواتير بصيغة <span class="font-mono font-bold">INV-OLD-YYYYMMDD-XXXXX</span></p>
                            </div>
                        </div>
                        <div class="bg-amber-50 dark:bg-amber-900/10 border border-amber-200 dark:border-amber-900/30 rounded-xl p-4">
                            <div class="flex gap-3">
                                ...
                                <div class="text-sm text-amber-800 dark:text-amber-300">
                                    ...
                                    <p>هذه الفواتير موثقة فور إنشائها وتُضاف مباشرة لدين المتجر.</p>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div x-show="editOpen" style="display:none" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-dark-card rounded-[2rem] p-8 shadow-2xl w-full max-w-md">
            ...
            <form :action="'{{ url('admin/old-debts') }}/' + editId" method="POST">
                @csrf
                @method('PATCH')
                <div class="mb-5">
    ...
    <div x-show="deleteOpen" style="display:none" class="fixed inset-0 z-50 flex items-center justify-center p-4">
        ...
        <div class="relative bg-white dark:bg-dark-card rounded-[2rem] p-8 shadow-2xl w-full max-w-md">
            ...
            <p class="text-gray-500 dark:text-gray-400 mb-6">هل أنت متأكد من حذف الفاتورة <span class="font-bold text-gray-900 dark:text-white" x-text="deleteNumber"></span>؟ سيتم حذفها من دين المتجر أيضاً.</p>
            <form :action="'{{ url('admin/old-debts') }}/' + deleteId" method="POST">
                @csrf
                @method('DELETE')
                <div class="flex gap-3">
