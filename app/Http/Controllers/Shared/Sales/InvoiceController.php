...
class InvoiceController extends Controller
{
    public function generateSalesInvoicePdf(SalesInvoice $sale)
    {
        ...
        $data = [
            ...
            'totalProducts' => $sale->items->sum(function($item) { return $item->quantity + $item->free_quantity; }),
            ...
            'labels' => [
                ...
                'totalProducts' => $arabic->utf8Glyphs('عدد المنتجات'),
    ...
    public function generateSalesInvoiceThermalPdf(SalesInvoice $sale)
    {
        $sale->load('items.product', 'store', 'marketer', 'keeper', 'rejectedBy');
        ...
        $thankYouText = $arabic->utf8Glyphs('شكراً لتعاملكم معنا');
        ...
        return $pdf->download('invoice-thermal-' . $sale->invoice_number . '.pdf');
    }
}
