...
class InvoiceController extends Controller
{
    public function generateRequestPdf(MarketerRequest $request)
    {
        ...
        $data = [
            ...
            'rejectedBy' => ($request->status === 'rejected' && $request->rejecter) ? $arabic->utf8Glyphs($request->rejecter->full_name) : null,
            'rejectedDate' => $request->status === 'rejected' ? $toEnglishNumbers($request->rejected_at ? $request->rejected_at->format('Y-m-d H:i') : $request->updated_at->format('Y-m-d H:i')) : null,
            ...
            'labels' => [
                ...
                'rejectedDate' => $arabic->utf8Glyphs('تاريخ الرفض'),
